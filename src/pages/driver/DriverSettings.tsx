...
export const DriverSettings: React.FC = () => {
  ...
  const handleUpdatePassword = async (e: React.FormEvent) => {
    ...
    if (newPassword.length < 6) {
      setPasswordError(t('Minimum 6 characters'));
      return;
    }

    if (newPassword !== confirmPassword) {
      setPasswordError(t('Passwords do not match. Please verify and try again.'));
      return;
    }
    ...
    try {
      ...
      setPasswordSuccess(t('Preferences saved successfully!'));
      setNewPassword('');
      setConfirmPassword('');
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-brand-navy font-display">{t('Account Settings')}</h1>
        <p className="text-xs text-slate-500 mt-1">{t('Manage notification alerts and account security credentials')}</p>
      </div>
      ...
      {saved && (
        <div className="flex items-center gap-2 p-3.5 bg-emerald-50 text-emerald-800 rounded-xl border border-emerald-200 text-xs font-bold animate-in fade-in shadow-xs">
          <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" /> {t('Preferences saved successfully!')}
        </div>
      )}
      ...
      <form onSubmit={handleSavePreferences} className="space-y-6">
        <div className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-card space-y-4">
          <h2 className="text-sm font-bold text-brand-navy uppercase tracking-wider flex items-center gap-2">
            <Bell className="w-4 h-4 text-brand-amber" /> {t('Job Alerts & Updates')}
          </h2>

          <div className="space-y-3 divide-y divide-slate-100">
            <label className="flex items-center justify-between pt-2 first:pt-0 cursor-pointer">
              <div>
                <p className="text-xs font-bold text-slate-900">{t('WhatsApp Instant Alerts')}</p>
                <p className="text-[11px] text-slate-500">{t('Get notified immediately when an employer shortlists you')}</p>
              </div>
              <input
                type="checkbox"
            ...
            <label className="flex items-center justify-between pt-3 cursor-pointer">
              <div>
                <p className="text-xs font-bold text-slate-900">{t('SMS Notifications')}</p>
                <p className="text-[11px] text-slate-500">{t('Receive interview schedules and venue details via SMS')}</p>
              </div>
              <input
                type="checkbox"
                checked={smsAlerts}
            ...
            <label className="flex items-center justify-between pt-3 cursor-pointer">
              <div>
                <p className="text-xs font-bold text-slate-900">{t('Email Digest')}</p>
                <p className="text-[11px] text-slate-500">{t('Weekly digest of high-paying jobs in your city')}</p>
              </div>
              <input
                type="checkbox"
                checked={emailAlerts}
          ...
          <div className="pt-2 flex justify-end">
            ...
            >
              <Save className="w-3.5 h-3.5 text-brand-amber" /> {t('Save Alert Preferences')}
            </button>
          </div>
        </div>
      </form>
      ...
      <div className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-card space-y-4">
        <h2 className="text-sm font-bold text-brand-navy uppercase tracking-wider flex items-center gap-2">
          <Lock className="w-4 h-4 text-brand-blue" /> {t('Password & Security')}
        </h2>
        ...
        <form onSubmit={handleUpdatePassword} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1.5">{t('New Password')}</label>
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1.5">{t('Confirm New Password')}</label>
              <div className="relative">
                <input
          ...
          <div className="flex justify-end pt-1">
            ...
            >
              {passwordLoading ? (
                <>
                  <Loader2 className="w-3.5 h-3.5 animate-spin" /> {t('Updating Password...')}
                </>
              ) : (
                <>
                  <Shield className="w-3.5 h-3.5 text-brand-amber" /> {t('Update Password')}
                </>
              )}
            </button>
          </div>
      ...
      <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 flex items-center justify-between">
        <div>
          <p className="text-xs font-bold text-slate-900">{t('Reset Demo Database State')}</p>
          <p className="text-[11px] text-slate-500">{t('Reverts all jobs, applications, and drivers to default seed')}</p>
        </div>
        <button
          type="button"
          onClick={() => {
        ...
        >
          <RotateCcw className="w-3.5 h-3.5" /> {t('Reset Demo Data')}
        </button>
      </div>
    </div>
  );
